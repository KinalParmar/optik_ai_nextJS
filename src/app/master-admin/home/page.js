...
} from "@/src/Services/Master-Admin/Home";
...
export default function Home() {
  ...
  const totalLeads = stats.reduce(
    (sum, stat) => sum + (stat.leadsCount || 0),
    0
  );
  const totalUsers = stats.reduce(
    (sum, stat) => sum + (stat.userCount || 0),
    0
  );
  ...
  const getAllCompanyDetails = async () => {
    try {
      ...
      if (response) {
        const uniqueCompanies = Array.isArray(response)
          ? response.filter(
              (company, index, self) =>
                company?._id &&
                  ...
                  <div className="mb-6">
                    ...
                    <div className="space-y-4">
                      ...
                      <p className="pb-2 border-b border-[#EEEEEE]">
                        ...
                        {selectedCompany?.admin?.countryCode &&
                        selectedCompany?.admin?.phoneNumber
                      ...
                      <p className="pb-2 border-b border-[#EEEEEE]">
                        ...
                        Users:{" "}
                        {selectedCompany?.admin?.permissions?.users?.join(
                          ", "
                        ) || "-"}
                        , Leads:{" "}
                        {selectedCompany?.admin?.permissions?.leads?.join(
                          ", "
                        ) || "-"}
                      </p>
                      <p className="pb-2 border-b border-[#EEEEEE]">
                      ...
                      <p className="pb-2 border-b border-[#EEEEEE]">
                        ...
                        {selectedCompany?.isEnabled !== undefined
                          ? selectedCompany.isEnabled.toString()
                          : "-"}
                      </p>
                      <p className="pb-2 border-b border-[#EEEEEE]">
                        <strong className="text-[#334155] font-extrabold">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <div className="w-24">
                        ...
                        {errors?.adminCountryCode && (
                          <p className="text-red-500 text-xs mt-1">
                            {errors?.adminCountryCode?.message}
                          </p>
                        )}
                      </div>
