...
export default function AdminLogin() {
    ...
    try {
      ...
      if (response?.success) {
        ...
        if (!user?.isAdmin && !user?.isVerified) {
          setShowPasswordModal(true); // Show modal if not admin and not verified
        } else {
  ...
  // Handle password creation submission (placeholder logic)
  const onPasswordSubmit = async (data) => {
    try {
      setLoading(true);
      // Add API call here to update password and verify user
      console.log('New Password Data:', data);
      // Example: await UpdatePasswordApi({ email: user.email, password: data.createPassword });
          ...
          <div className="bg-white rounded-lg w-[450px] max-w-[90%] max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex justify-between items-center mb-6">
                <h4 className="text-xl font-extrabold text-[#334155]">Set Your Password</h4>
                <button
              ...
              <form onSubmit={handlePasswordSubmit(onPasswordSubmit)} className="space-y-5">
                <div>
                  <label className="text-[#334155] font-semibold">Create Password</label>
                  <input
                    type="password"
                    {...registerPassword('createPassword')}
                    className={`w-full h-11 px-3 mt-2 rounded-md bg-white text-[15px] placeholder-gray-400 border ${passwordErrors?.createPassword ? 'border-red-500' : 'border-gray-300'
                      } focus:outline-none focus:border-[#007BFF] focus:ring-1 focus:ring-[#007BFF]`}
                  />
                  {passwordErrors?.createPassword && (
                    <p className="text-red-500 text-sm mt-1">{passwordErrors?.createPassword?.message}</p>
                  )}
                </div>

                <div>
                  <label className="text-[#334155] font-semibold">Confirm Password</label>
                  <input
                    type="password"
                    {...registerPassword('confirmPassword')}
                    className={`w-full h-11 px-3 mt-2 rounded-md bg-white text-[15px] placeholder-gray-400 border ${passwordErrors?.confirmPassword ? 'border-red-500' : 'border-gray-300'
                      } focus:outline-none focus:border-[#007BFF] focus:ring-1 focus:ring-[#007BFF]`}
                  />
